...
FormValidator.prototype={texts:{invalid:'Entrada inesperada',short:'Los datos de entrada son muy cortos',long:'Los datos de entrada son demasiados',checked:'Por favor verifique los datos',empty:'El campo es obligatorio, no puede ir vacio',select:'Por favor, seleccione una opcion',number_min:'El dato es muy pequeño',number_max:'El dato es muy grande',url:'Ingrese una URL valida',number:'La entrada solo acepta numero',email:'Ingrese un Mail valido',email_repeat:'El Mail introducido no coincide',date:'Fecha invalida',time:'Hora Invalida',password_repeat:'Las contraseñas no coinciden',no_match:'No coincide',complete:'Los datos estan incompletos'},defaults:{alerts:true,events:false,regex:{url:/^(https?:\/\/)?([\w\d\-_]+\.+[A-Za-z]{2,})+\/?/,phone:/^\+?([0-9]|[-|' '])+$/i,numeric:/^[0-9]+$/i,alphanumeric:/^[a-zA-Z0-9]+$/i,email:{illegalChars:/[\(\)\<\>\,\;\:\\\/\"\[\]]/,filter:/^.+@.+\..{2,6}$/}},classes:{item:'field',alert:'alert',bad:'bad'}},tests:{sameAsPlaceholder:function(field,data){if(field.getAttribute('placeholder'))
